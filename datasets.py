...
class FrameVideoDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Load optical flow if requested (for dual-stream networks)
        if self.load_optical_flow:
            flow_dir = self.video_paths[idx].split('.avi')[0].replace('videos', 'flows-png')
            flow_frames = self.load_frames(flow_dir, is_flow=True)
    ...
    def load_frames(self, frames_dir, is_flow=False):
        frames = []
        ext = '.png' if is_flow else '.jpg'
        prefix = 'flow_' if is_flow else 'frame_'

        for i in range(1, self.n_sampled_frames + 1):
            frame_file = os.path.join(frames_dir, f"{prefix}{i}{ext}")
